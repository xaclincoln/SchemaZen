...
";
        ...
        public void ScriptToFile(string tableHint = null, Action<TraceLevel, string> log = null)
        {
            var file = new StreamWriter(new FileStream(File, FileMode.Create, FileAccess.Write));

            file.WriteLine("CREATE DATABASE {0}", Name);
            file.WriteLine();
            file.WriteLine("GO");
            file.WriteLine("USE {0}", Name);
            file.WriteLine();
            file.WriteLine("GO");
            file.WriteLine();
            ...
            if (Props.Count > 0)
            {
                file.WriteLine(ScriptPropList(Props));
                file.WriteLine("GO");
                file.WriteLine();
            }
            ...
            foreach (var schema in Schemas)
            {
                file.WriteLine(schema.ScriptCreate());
                file.WriteLine("GO");
                file.WriteLine();
            }
            ...
            foreach (var fk in ForeignKeys)
            {
                file.WriteLine(fk.ScriptCreate());
            }
            file.WriteLine();
            file.WriteLine("GO");
            ...
            foreach (var u in Users)
            {
                if (string.Equals(u.Name, "dbo", StringComparison.CurrentCultureIgnoreCase))
                {
                    continue;
                }
                file.WriteLine(u.ScriptCreate());
                file.WriteLine();
                file.WriteLine("GO");
            }
        ...
        private void WritePropsScript(Action<TraceLevel, string> log)
        {
            ...
            text.AppendLine("GO");
            text.AppendLine();
            System.IO.File.WriteAllText(string.Format("{0}/props.sql", Dir), text.ToString());
        }
        ...
        private void WriteSchemaScript(Action<TraceLevel, string> log)
        {
            ...
            text.AppendLine("GO");
            text.AppendLine();
            System.IO.File.WriteAllText(string.Format("{0}/schemas.sql", Dir), text.ToString());
        }
        ...
        private void WriteScriptDir(string name, ICollection<IScriptable> objects, Action<TraceLevel, string> log)
        {
            ...
            foreach (var o in objects)
            {
                ...
                System.IO.File.AppendAllText(filePath, script);
            }
        }
        ...
        public void ExportData(string tableHint = null, Action<TraceLevel, string> log = null)
        {
            ...
            foreach (var t in DataTables)
            {
                ...
                var sw = System.IO.File.CreateText(filePathAndName);
        ...
        public void CreateFromDir(bool overwrite, string databaseFilesPath = null, Action<TraceLevel, string> log = null)
        {
            ...
            //run scripts
            if (System.IO.File.Exists(Dir + "/props.sql"))
            {
                log(TraceLevel.Verbose, "Setting database properties...");
                try
                {
                    DBHelper.ExecBatchSql(Connection, System.IO.File.ReadAllText(Dir + "/props.sql"));
                }
                catch (SqlBatchException ex)
                {
            ...
            if (System.IO.File.Exists(Dir + "/schemas.sql"))
            {
                log(TraceLevel.Verbose, "Creating database schemas...");
                try
                {
                    DBHelper.ExecBatchSql(Connection, System.IO.File.ReadAllText(Dir + "/schemas.sql"));
                }
                catch (SqlBatchException ex)
                {
            ...
            while (scripts.Count > 0 && (prevCount == -1 || errors.Count < prevCount))
            {
                ...
                foreach (var f in scripts.ToArray())
                {
                    ...
                    try
                    {
                        DBHelper.ExecBatchSql(Connection, System.IO.File.ReadAllText(f));
                        scripts.Remove(f);
                    }
                    catch (SqlBatchException ex)
                    {
